...
int match__eq_state(const node_t *data, void *info)
{
    ...
    struct info_build_ec *user = ii->info;
...
void lexer__minimize(node_t *dst)
{
    ...
    while (changes) {
        ...
        while (states != NULL) {
            ...
            // build all equivalence classes for this state and push them to ecq
            node_t ec = NULL;
        ...
        sp__access(U_QT, &ec, &en, sp__predicate_1, pretty_print_states);



                // initialize static list of states
                node_t static_states = NULL;
                lexer__access(U_QT, dst, &static_states, predicate_1, apply__get_states);

                // in whole graph replace class nodes with representative
                sp__access(U_QT, &static_states, &ec, sp__predicate_1, apply__replace_node_refs_nreps);
